...
export default class Day extends Component {
    ...
    render() {
        let {caption, customStyle, viewForDay} = this.props;
        const {
            current,
            filler,
            event,
        ...
        const text = viewForDay
            ? (
                viewForDay(current, isWeekend, isSelected, isToday, event, caption)
            )
            : (
        ...
        const weekendStyles = isWeekend ? [styles.weekendDayButton, customStyle.weekendDayButton] : [];
        const indicators = showEventIndicators
            ? (
                <View style={[
        ...
        return filler
            ? (
                <TouchableWithoutFeedback>
                    <View style={[styles.dayButtonFiller, {width: containerWidth / 7}, customStyle.dayButtonFiller]}>
            ...
            : (
                <TouchableOpacity onPress={this.props.onPress}>
                    <View style={[styles.dayButton, {width: containerWidth / 7}, customStyle.dayButton, ...weekendStyles]}>
                        {text}
                        {indicators}
                    </View>
                </TouchableOpacity>
